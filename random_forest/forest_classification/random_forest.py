...
# Configurações
from forest_classification.settings import get_path_to_data
...
# -----------------------------------
# 4. AJUSTE DO MODELO RANDOM FOREST
# -----------------------------------
print("\n🌲--- 4. AJUSTE DO MODELO RANDOM FOREST ---🌲")
...
rf_classifier.fit(X_train, y_train)  # Ajusta o modelo aos dados de treinamento
training_time = time.time() - start_time

# Prever resultados
y_pred = rf_classifier.predict(X_test)  # Previsão com o conjunto de teste
...
performance_table.add_row(["Número de Árvores", len(rf_classifier.estimators_)])
performance_table.add_row(["Profundidade Média das Árvores", sum([tree.tree_.max_depth for tree in rf_classifier.estimators_]) / len(rf_classifier.estimators_)])
...
print(f"   ✅ Sim, o modelo Random Forest foi treinado e testado com uma precisão média de {accuracies:.2f}.")
